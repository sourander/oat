...
| **Määritelmät**             | Keskeiset käsitteet ja ilmiöt | Miten määrittelet aiheen tärkeimmät termit? Voit käyttää taulukoita, kuvaajia, lainauksia jne.                            |            200–400 |
| **Käsittelyosa & Tehtävät** | Merkinnän varsinainen pääosa  | Mitä teit, mitä opit, mitä oivalsit? Käytä kuvia, kaavioita, koodilohkoja, listoja, taulukoita, lainauksia, linkkejä jne. |            300–600 |
...
Oppimispäiväkirjaa voi kirjoittaa tehtävien osalta kahdella tavalla. 

1. **Tee erillinen Tehtävät-otsikko teorian perään.** 
    
    Tämä on koulumainen tapa, joka on monille opiskelijoille tutuin. Tehtävät käsitellään erillisenä osiona, joka on otsikoitu esimerkiksi "Tehtävät". Tällöin tehtävien käsittely on selkeästi erotettu teorian käsittelystä. 
    
    Vaarana on, että alusta tulee ikään kuin irrallinen, ChatGPT:n suoltama täysin teoreettinen teksti, joka on sinun osaamisestasi irrallaan. Jos käytät tätä tapaa, kytke ne tekstillä toisiinsa. Teorian lomassa voit sanoa, että *"Tätä näkyy tehtävässä X siten, että ..."*. Vastavuoroin tehtävän vastauksen yhteydessä voit kirjoittaa, että *"Tämä liittyy yllä esiteltiin Y:hyn siten, että ..."*.

2. **Upota eli integroi vastaukset oppimispäiväkirjan käsittelyosaan.**

    Tämä keino vaatii enemmän kirjoitustaitoa, mutta tuottaa eheämmän tekstin. Tehtävät käsitellään ikään kuin case-esimerkkeinä, jotka ovat upotettuina teorian lomaan. Jos luet esimerkiksi Géronin [Hands-On Machine Learning with Scikit-Learn and PyTorch](https://kamk.finna.fi/Record/kamk.99669144906247?sid=5336515502)-kirjaa, huomaat, että täysin erillistä teoriaosuutta ei ole olemassa. Erityisesti alkuosan luvut **The Machine Learning Landscape** ja **End-to-End Machine Learning Project** ovat käytännössä tehtävien käsittelyä, joka on upotettu teorian lomaan. Tämä on erinomainen esimerkki siitä, miten tehtävät voidaan integroida osaksi oppimispäiväkirjaa.

Osa tehtävistä voi olla tehty hyvinkin lyhyesti: pelkkä yksi kuva ja kuvateksti saattaa riittää yksinkertaisimmissa tehtävissä. Toisissa tehtävissä, kuten vaativammissa ohjelmointitehtävissä, saattaa olla perusteltua kirjoittaa pidempi teksti. Tehtävät eivät ole kuitenkaan mikään valinnainen osio: ne tulee tehdä, ja niiden tekemisestä tulee antaa todisteita oppimispäiväkirjassa.
...
Varmista kuitenkin, että kukin teemamerkintä on laajuudeltaan aivan vähimmillään noin **1 liuskan eli 500 sanaa**.  On täysin realistista, joskin haastavaa, että voit saavuttaa täyden arvosanan (5/5) noin 600 sanalla (per merkintä). Ota huomioon, että tähän ei lasketa kuvia ja koodilohkoja; niiden tehokas käyttö on oiva tapa sisällyttää paljon tietoa lyhyeen tekstiin.
